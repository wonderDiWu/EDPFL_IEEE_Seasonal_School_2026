...
class IID_Generator(Data_Generator):
    def __init__(self, cpu_count, dataset, num_clients, num_shards, shard_indices, SEED, PATH, B):
        ...
        for i in range(num_clients):
            ...
            self.clients_data.append(MyDataset(shards_data, shards_targets, self.transform_train))
               
   
    def get_local_dataloader(self, client_id):
        trainloader = DataLoader(
        self.clients_data[client_id], batch_size=self.B, shuffle=True, num_workers=self.cpu_count)
        return (client_id, trainloader)
...
def unit_test():
    cpu_count, dataset, num_clients, num_shards = 1, 'CIFAR10', 100, 200
    random.seed(0)
    shard_indices = random.sample(range(num_shards), num_shards)
 
    iid_generator = IID_Generator(cpu_count, dataset, num_clients, num_shards, shard_indices)
